...
class CRUDService:
    ...
    def get(self, item_id: int):
        ...
        if obj is None:
            raise NotFoundError(
                f"{self.config.name.replace('-', ' ').title()} not found"
            )
        return obj
    ...
    def update(self, item_id: int, payload: BaseModel, actor_id: int | None):
        ...
        if (
            self.config.model is Account
            and values.get("account_number")
            and self.session.scalar(
                select(Account.id).where(
                    Account.account_number == values["account_number"],
                    Account.id != item_id,
                )
            )
        ):
    ...
    def _normalize_values(self, values: dict[str, Any], obj=None) -> dict[str, Any]:
        ...
        details.update(
            {key: value for key, value in values.items() if key not in columns}
        )
        ...
        if obj is None and self.config.model is Account:
            normalized["account_number"] = (
                ...
                or f"FMS-{str(normalized.get('account_type') or self.config.fixed_values.get('account_type') or 'ACCOUNT').upper()[:6]}-{uuid4().hex[:8].upper()}"
            )
            normalized["name"] = (
                normalized.get("name")
                or " ".join(
