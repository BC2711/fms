...
Set `FMS_SEED_ON_STARTUP=true` once for local bootstrap, or run `python -m app.utilities.seed_cli`. The initial administrator is 
`admin@fms.example.com` / `ChangeMe123!`; change it immediately outside local development.
...
See [Adding a Backend Module](docs/ADDING_A_BACKEND_MODULE.md) for the complete step-by-step guide, including migrations, permissions, tests, and custom workflows.
